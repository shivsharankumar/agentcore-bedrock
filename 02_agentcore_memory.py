...
REGION = "ap-south-1"
MEMORY_ID = "customercareagentmemory-eOBzZWDzYW"
...
agent = create_agent(
    model=llm,
    tools=tools,
    checkpointer=checkpointer,#short term memory specified by checkpointer (for single conversation)
    store=store,#long term memory specified by store using thread
    middleware=[MemoryMiddleware()],#add previous context with new context
